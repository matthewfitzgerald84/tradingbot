...
symbols = get_symbols(bitstamp)

since = bitstamp.parse8601('2022-01-01T00:00:00Z')

ohlcv_data = fetch_historical_data(bitstamp, symbol, '1d', since, 1000)

returns_df = create_returns_df(ohlcv_data)

top_symbols = select_top_symbols(crypto_market_df, NUM_TOP_SYMBOLS)
...
def fetch_historical_data(ccxt_exchange, symbol, timeframe, since, limit):
    data = ccxt_exchange.fetch_ohlcv(symbol, timeframe, since, limit)
    headers = ['timestamp', 'open', 'high', 'low', 'close', 'volume']
    return pd.DataFrame(data, columns=headers)

def select_top_symbols(df, num_symbols):
    return df.nlargest(num_symbols, 'market_cap').index.tolist()
...
def updated_optimize_portfolio(returns_df, symbols):
    expected_returns_annual = expected_returns.mean_historical_return(returns_df)
    ...
    pe_ratios = [fetch_fundamental_data(symbol).get('trailingPE', np.nan) if fetch_fundamental_data(symbol) is not None else np.nan for symbol in symbols]
    ...
    fundamental_data_list = [fetch_fundamental_data(symbol) for symbol in symbols]
    ...
    weight_dict.update(dict(zip(symbols, normalized_combined_weights)))

    return weight_dict

def fetch_asset_fundamental_data(symbol):
...
def main():
    ...
    returns_df = fetch_historical_data(bitstamp, symbols, interval)
    top_symbols = select_top_symbols(returns_df, 10)

    optimized_portfolio = updated_optimize_portfolio(returns_df, symbol_list)

    for symbol in symbols:
        short_mavg, long_mavg = calculate_moving_averages(bitstamp, symbol, interval, short_window, long_window)
        print(f"{symbol}: Short moving average: {short_mavg:.2f}, Long moving average: {long_mavg:.2f}")
...
while True:
        ...
        for symbol in optimized_portfolio:
        ...
        if first_run:
            ...
            current_distribution = get_current_distribution(exchange, symbols)
            if should_rebalance(current_distribution, optimized_portfolio):
                rebalance_portfolio(exchange, symbols, optimized_portfolio)
